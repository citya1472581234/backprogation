...
def conv_forward_strides(x, w, b, conv_param):
    ...
    shape = (C, int(HH), int(WW), N, int(out_h), int(out_w))
    strides = (int(H * W), int(W), 1, int(C * H * W), int(stride * W), stride)
    strides = int(x.itemsize) * np.array(strides)
