...
def train(model, train_set, test_set, save, n_epochs=100, valid_size=5000,
    ...
    for epoch in range(n_epochs):
        ...
        if valid_loader and valid_error < best_error:
            ...
            torch.save(model.state_dict(), os.path.join(save, 'LSTM_classifier.dat'))
        else:
            torch.save(model.state_dict(), os.path.join(save, 'LSTM_classifier.dat'))
    ...
    model.load_state_dict(torch.load(os.path.join(save, 'LSTM_classifier.dat')))
    if torch.cuda.is_available() and torch.cuda.device_count() > 1:
    ...
    with open(os.path.join(save, 'LSTM_classifier_results.csv'), 'a') as f:
        f.write(',,,,,%0.5f\n' % (test_error))
    print('Final test error: %.4f' % test_error)

def demo(data, save, valid_size=5000,n_epochs=100, batch_size=64, seed=None):
    """
    ...
    class RNN(nn.Module):
        ...
        def forward(self, x):
             ...
             out = self.out(r_out[:, -1, :]) # out shape (batch, time_step, input_size)
             return out
    model =RNN()
    print(model)
